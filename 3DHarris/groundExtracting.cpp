                                              ...
                                              int row, int list, int num_voxel, Voxel* grid)
{
    ...
    for (int i = 0; i < cloud->points.size(); i++)
    {
        ...
        {
            grid[temp_num].point_id.push_back(i);//这个i就是点云的index，存储在grid[].point_id里面
    ...
    for (int i = 0; i < num_voxel; i++)
    {
        for (int j = 0; j < grid[i].point_id.size(); j++)
        {
            if (cloud->points[grid[i].point_id[j]].z - grid[i].min_z < max_height_difference_&&grid[i].min_z - grid[i].NeighborMin_z < 2 * grid_resolution_)
            {
                ...
                groundpts_indexs.push_back(grid[i].point_id[j]);//自己加的index
            }
            else
            {
                                             ...
                                             pcl::PointCloud<pcl::PointXYZI>::Ptr no_ground_cloud)
{
    ...
    for (int i = 0; i < ground_cloud->points.size(); i++)
    {
        ...
        temp_ground_cloud->push_back(groundpointsTotempgroudpoints);//存放的是点的坐标
        is_ground_points.push_back(i);//存放的是点的id——i，而不是实际的点坐标
    }
    ground_cloud->clear();
    set<int, less<int>>Unsearch;//存放未经过搜索的点的id——i，有大小顺序
    set<int, less<int>>::iterator iterUnsearch;
    vector<int>isearse;//存放点的id——i
    ...
    while (!Unsearch.empty())
    {
        ...
        if (PointIdSearch_ground_cloud.size() < 5)//5
        {
        ...
        {
            if (PointIdSearch_ground_cloud.size() > 10)//10
            {
                                           ...
                                           Bounds_ym bounds, CenterPoint_ym center_pt,vector<int> &groundpts_indexs)
{
    ...
    /*根据高程差值判断地面点;*/
    JudgeGroundAndNonGroundPoints(cloud, ground_cloud, no_ground_cloud, grid, num_voxel,groundpts_indexs);
    /*滤除误分非地面点*/
