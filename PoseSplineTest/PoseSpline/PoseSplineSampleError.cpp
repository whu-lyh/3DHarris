...
PoseSplineSampleError::PoseSplineSampleError(double t_meas, Pose<double> T_meas)
...
{
    // convert parameter to pose(in fact the parameter array is from pose...)
    // 0-3 corresponding to 4 basic functions from 0-degree to 3-degree (total 4 order)
    ...
    // all same as the slerp interpolation of quaternion
    // some hints from https://www.guyuehome.com/18692
    // betai is the value at the knot position of each i-order basic function
    double Beta1 = QSUtility::beta1(t_meas_);
    double Beta2 = QSUtility::beta2(t_meas_);
    double Beta3 = QSUtility::beta3(t_meas_);
    // like position: the relative angle is from Phi() function
    ...
    // like position: the each part of lower order of basic function
    ...
    // For simplity, we define error  =  /hat - meas. estimate - measure
    ...
    // t_error = t_hat(estimate) - T(measure)
    ...
    // the jacobians for each control points
    if (jacobians != NULL)
    {
        Eigen::Matrix<double, 6, 7> J_1st;
        J_1st.setIdentity();
        J_1st.bottomRightCorner(3, 4) = 2.0 * quatRightComp(quatInv(T_Meas_.rotation())).topLeftCorner(3, 4);
